...
async function main() {
  ...
  let written = 0;
  let errored = 0;
  const permanentlyFailed = []; // record objects that gave up after 5 attempts

  const recordById = new Map(); // for the retry pass below
  ...
  bulkWriter.onWriteError((error) => {
    ...
    const id = error.documentRef.id;
    console.error(`Permanently failed after 5 attempts: records/${id}`, error.message);
    if (recordById.has(id)) permanentlyFailed.push(recordById.get(id));
    return false;
  });
  ...
  for (const row of rows) {
    ...
    // Catch here too: onWriteError returning false intentionally rejects
    // this specific write's promise. Without a .catch() attached directly,
    // that becomes an unhandled rejection and crashes the process at the
    // very end — after all the real work is already done. Harmless to the
    // data, but ugly, so we swallow it here (onWriteError already logged it).
  ...
  console.log(`\nMain pass done in ${totalTime}s. Queued: ${written}, permanent errors: ${errored}.`);

  if (permanentlyFailed.length) {
    console.log(`\nRetrying ${permanentlyFailed.length} permanently-failed record(s) in a fresh, smaller pass...`);
    const retryWriter = db.bulkWriter();
    let retrySucceeded = 0;
    let retryFailed = 0;
    retryWriter.onWriteError((error) => {
      if (error.failedAttempts < 5) return true;
      retryFailed++;
      console.error(`Retry also failed for records/${error.documentRef.id}:`, error.message);
      return false;
    });
    for (const record of permanentlyFailed) {
      const ref = db.collection('records').doc(record.id);
      retryWriter
        .set(ref, record, { merge: true })
        .then(() => { retrySucceeded++; })
        .catch(() => {});
    }
    await retryWriter.close();
    console.log(`Retry pass done. Succeeded: ${retrySucceeded}, still failed: ${retryFailed}.`);
  }

  // Hard verification — an actual count from Firestore itself, not just
  // trust in the log lines above.
  console.log('\nVerifying final document count in Firestore (this may take a few seconds)...');
  const countSnapshot = await db.collection('records').count().get();
  console.log(`records collection now contains: ${countSnapshot.data().count} documents`);
  console.log(`(Expect somewhat under ${written} — a handful of exact-duplicate rows in the source data intentionally collapse to the same document ID, which is correct behavior.)`);
}
