...
export function makeRecord(input) {
  ...
  return {
    ...
    // Physical storage location. Updated (Sep 2026): the Plant 2 -> Plant 1
    // workflow is now live. New samples arrive at Plant 2 first and get
    // manually moved to Plant 1 later (via the location action buttons /
    // setLocation) once they're transferred for long-term storage. Records
    // created BEFORE this change that have no `location` field at all are
    // still treated as Plant 1 by every read-path fallback in this file
    // (`r.location || 'Plant 1'`) and in QC2.dc.html — this default only
    // affects records created from here on.
    location: (input.location || 'Plant 2').trim() || 'Plant 2',
...
export async function putMany(records, onProgress, opts = {}) {
  ...
  if (fs && failedCount === 0 && (opts.isNew || opts.statusDelta || opts.locationDelta)) {
    try {
      ...
      if (opts.isNew) {
        update.total = FV.increment(records.length);
        // Same tally-then-emit pattern as the small-batch path above, and
        // for the same reason: also track statuses/locations, not just
        // groups, or those dashboard breakdowns silently drift behind on
        // every large merge (anything >10 changed records) done via the
        // app's own import/merge UI.
        const tally = {};
        for (const r of records) {
          const bump = k => { tally[k] = (tally[k] || 0) + 1; };
          bump(`groups.${r.group}`);
