...
//   LEGACY_LOCATION  — how to read a record that has no `location` field at
//                      all. Everything imported from the old Access database
//                      predates the field and physically lives at Plant 1.
//   NEW_LOCATION     — where a brand-new sample starts. The Plant 2 ->
//                      Plant 1 workflow is live: samples are received at
//                      Plant 2, then manually moved to Plant 1 for
//                      long-term storage via setLocation().
export const LEGACY_LOCATION = 'Plant 1';
export const NEW_LOCATION = 'Plant 2';
...
export function makeRecord(input) {
  ...
  return {
    ...
    // An incoming `id` means we are rebuilding an EXISTING record (a status
    // change, an edit, a backup restore) — those must keep the legacy
    // Plant 1 reading when they carry no location, or marking an old sample
    // as "dipinjam" would silently relocate it to Plant 2. Only a record
    // with no id yet is genuinely new, and those start at Plant 2.
    location: (input.location || (input.id ? LEGACY_LOCATION : NEW_LOCATION)).trim()
      || (input.id ? LEGACY_LOCATION : NEW_LOCATION),
...
/**
 * Fetches EVERY non-deleted record for a full export.
 *
 * loadAll() above deliberately returns [] so state.records stays empty
 * in server mode — that's correct for boot, but it means state.records
 * is never a valid source for "Export full backup" / "Export all (.csv)":
 * exporting it just produces an empty file every time, no matter how
 * many records are in Firestore. This pages through queryRecords()
 * instead, sorted by batch (the app's stable unique key, per
 * mergeRecords() below) so cursor pagination doesn't skip or repeat
 * rows, until every page is exhausted.
 *
 * Returns null when there's no Firestore connection at all, so the
 * caller can fall back to whatever it already has in memory.
 */
export async function loadAllForExport(onProgress) {
  const fs = getFirestore();
  if (!fs) return null;
  ...
  const pageSize = 1000;
  for (;;) {
    const res = await queryRecords({ sort: 'batch-asc', pageSize, cursor });
    for (const r of res.rows) if (!r.deleted) rows.push(r);
    if (typeof onProgress === 'function') onProgress(rows.length);
    if (res.exhausted || !res.nextCursor) break;
...
/**
 * Builds the meta/stats increment payload.
 *
 * IMPORTANT — this is the fix for the Dipinjam counter never moving.
 * Firestore's set() treats a key containing a dot as a LITERAL field name.
 * `set({ 'statuses.borrowed': increment(1) }, { merge: true })` therefore
 * creates a junk top-level field actually called "statuses.borrowed" and
 * leaves the real nested `statuses.borrowed` untouched — so the sidebar
 * badge and the dashboard status card never changed after a borrow or a
 * return. Only update() reads dots as a path, but update() throws if the
 * doc doesn't exist yet, which we can't guarantee.
 *
 * So: build a genuinely nested object. FieldValue.increment works fine
 * nested inside set({merge:true}), and merge still creates the doc on
 * first write. Best of both.
 *
 * Counters are tallied as plain numbers first and converted to increments
 * only at the end, so two sources touching the same counter add up instead
 * of the second one overwriting the first.
 */
function buildStatsUpdate(records, opts) {
  const FV = window.firebase.firestore.FieldValue;
  const tally = { groups: {}, statuses: {}, locations: {} };
  const add = (parent, key, n) => {
    if (!key || !n) return;
    tally[parent][key] = (tally[parent][key] || 0) + n;
  };
  ...
  if (opts.isNew) {
    ...
    for (const r of records) {
      add('groups', r.group, 1);
      add('statuses', r.status || 'ok', 1);
      add('locations', r.location || LEGACY_LOCATION, 1);
    }
  }
  if (opts.statusDelta) {
    for (const [st, delta] of Object.entries(opts.statusDelta)) add('statuses', st, delta);
  }
  if (opts.locationDelta) {
    for (const [loc, delta] of Object.entries(opts.locationDelta)) add('locations', loc, delta);
  }

  const update = {};
  if (total) update.total = FV.increment(total);
  for (const [parent, counters] of Object.entries(tally)) {
    for (const [key, n] of Object.entries(counters)) {
      if (!n) continue;
      if (!update[parent]) update[parent] = {};
      update[parent][key] = FV.increment(n);
    }
  }
  return update;
}

async function writeStatsUpdate(fs, records, opts) {
  try {
    const update = buildStatsUpdate(records, opts);
  ...
  if (records.length <= 10 && fs) {
    try {
      ...
      if (opts.isNew || opts.statusDelta || opts.locationDelta) {
        await writeStatsUpdate(fs, records, opts);
      }
