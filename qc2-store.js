...
export async function countRecords({ group = 'All', from = '', to = '' } = {}) {
  ...
  // Unfiltered or group-only counts come straight from the stats doc — O(1).
  if (!from && !to) {
    try {
      ...
      if (doc.exists) {
        ...
        } else if (d.groups && typeof d.groups[group] === 'number') {
  ...
  // Date-filtered counts have no precomputed value — do a bounded count.
  // Capped low enough to stay fast; returns null if the cap is hit so the
  // UI can show "many" rather than a wrong number.
  try {
    let ref = fs.collection('records');
    if (group && group !== 'All') ref = ref.where('group', '==', group);
    if (from) ref = ref.where('date', '>=', from);
...
export async function rebuildStats() {
  ...
  const stats = { total, groups, boxCount: boxes.size, codeCount: codes.size, updatedAt: new Date().toISOString() };
