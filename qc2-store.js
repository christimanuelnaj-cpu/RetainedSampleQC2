...
// Bounded to the same recent slice as loadAll() — an unbounded live
// listener across 165k+ documents is prohibitively expensive to sync
// and was contributing to the same "records never appear" symptom.
export function subscribeRecords(onChange) {
  const fs = getFirestore();
  if (!fs) return () => {};
  try {
    return fs.collection('records')
      .orderBy('date', 'desc')
      .limit(3000)
      .onSnapshot(snap => {
        const records = [];
...
export async function loadAll() {
  ...
  if (fs) {
    try {
      // NOTE: bounded on purpose. At 165k+ documents, an unbounded
      // .get() on the whole collection was silently failing (too large /
      // timing out), which is why records stopped appearing entirely.
      // This loads the most recent slice reliably; full historical
      // search across all records needs proper filtered/paginated
      // queries wired to the search UI as a follow-up, not a one-line fix.
