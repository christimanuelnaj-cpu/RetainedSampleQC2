...
export async function countRecords({ group = 'All', from = '', to = '' } = {}) {
  ...
  if (!fs) return null;
  try {
    // compat SDK (loaded via script tag) does not support .count() aggregation.
    // Use a plain bounded get() instead — cheap since we only need .size.
    let ref = fs.collection('records');
    if (group && group !== 'All') ref = ref.where('group', '==', group);
    if (from) ref = ref.where('date', '>=', from);
    if (to) ref = ref.where('date', '<=', to);
    const snap = await ref.limit(200000).get();
    return snap.size;
  } catch (e) {
