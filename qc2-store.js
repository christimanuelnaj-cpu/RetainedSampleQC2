...
export async function queryRecords({
  ...
  // When a date range OR a status filter is active, Firestore requires
  // that field to lead the orderBy — so date wins over the user's chosen
  // sort there. (Also keeps the index count sane: without this, filtering
  // by status while sorted by batch/code/box would need 6+ more composite
  // indexes to cover every combination.)
  let [field, dir] = sortFieldFor(sort);
  if (((from || to) || status) && field !== 'date') { field = 'date'; dir = 'desc'; }
 ...
 */
export async function countRecords({ group = 'All', from = '', to = '', status = '' } = {}) {
  const fs = getFirestore();
  if (!fs) return null;

  // Unfiltered, group-only, or status-only counts come straight from the
  // stats doc — O(1). Combined group+status isn't precomputed, falls
  // through to the bounded query below.
  if (!from && !to) {
    try {
      ...
      if (doc.exists) {
        ...
        } else if (group !== 'All' && !status && d.groups && typeof d.groups[group] === 'number') {
          return d.groups[group];
        } else if (group === 'All' && status && d.statuses && typeof d.statuses[status] === 'number') {
